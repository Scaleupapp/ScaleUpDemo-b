...
async function runWave3StateBoardLongTail(opts = {}) {
  ...
  // Step 1: seed long-tail state board taxonomy entries
  const boards = JSON.parse(
    fs.readFileSync(path.join(__dirname, 'data', 'wave3-state-boards-longtail.json'), 'utf8')
  );
  console.log(`\n=== Step 1: Seed ${boards.length} Wave 3 long-tail state board taxonomy entries ===`);
  console.log(`Boards covered: UP, Rajasthan, Gujarat, Kerala, AP, Telangana, WB`);
  ...
  console.log(`\n=== Step 2: Generate anchors for ${totalTopicCount} topics (LLM, est. $3-6) ===`);
  if (!skipPrompts && (await prompt('Proceed? (y/N) ')).toLowerCase() !== 'y') return;
  ...
  console.log(`\n=== Step 3: Generate bulk questions — ${jobs.length} batches (est. $20-30) ===`);
...
async function main() {
  ...
  try {
    await runWave3StateBoardLongTail();
  } finally {
