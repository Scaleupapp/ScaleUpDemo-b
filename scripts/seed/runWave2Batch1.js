...
async function runWave2Batch1(opts = {}) {
  ...
  // Step 1: seed topic taxonomies
  const topics = JSON.parse(
    fs.readFileSync(path.join(__dirname, 'data', 'wave2-topics.json'), 'utf8')
  );
  console.log(`\n=== Step 1: Seed ${topics.length} Wave 2 Batch 1 taxonomy entries ===`);
  ...
  // Step 2: anchor questions for newly seeded entries
  const targetKeys = topics.map(t => t.targetKey);
